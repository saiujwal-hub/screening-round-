...
$$\text{Invalid} = \mathcal{M}_{\text{Missing\_Op}} \cup \mathcal{M}_{\text{Missing\_S}} \cup \mathcal{M}_{\text{Dup}} \cup \mathcal{M}_{\text{Negative}} \cup \mathcal{M}_{\text{Cross}} \cup \mathcal{M}_{\text{Spike}}$$
1. **Operating Parameter Missingness ($\mathcal{M}_{\text{Missing\_Op}}$)**: Missing operating inputs ($V, I, T_{amb}, t$) render tests unprovable per challenge rules; they are imputed with training medians for safe inference and flagged `Invalid`.
2. **Sensor Dropout ($\mathcal{M}_{\text{Missing\_S}}$)**: Missing entries ($NaN$) in terminal probes $S_1, S_2, S_3$.
3. **Duplicate Test Runs ($\mathcal{M}_{\text{Dup}}$)**: Identical operating vectors $[V, I, T_{amb}, t]$ arising from data-logger collisions or test repetitions.
4. **Physical Plausibility Rule ($\mathcal{M}_{\text{Negative}}$)**: Active load testing cannot yield negative temperature rise above ambient ($S_i < 0$), catching unphysical readings (e.g. `TST-0213`, $S_2 = -1.55^\circ\text{C}$).
5. **Cross-Sensor Consistency ($\mathcal{M}_{\text{Cross}}$)**: Gross deviations from linear thermodynamic coupling between terminal probes ($S_3 \approx 1.27 S_1$, $R^2 = 0.983$; $\tau_{cross} = 1.25\times \max \text{res}$).
6. **Conduction Residuals ($\mathcal{M}_{\text{Spike}}$)**: Residual deviations exceeding $1.25\times$ baseline conduction tolerances from $\hat{S}_i(V, I, T_{amb}, t)$ ($\tau_{S1}=0.86^\circ\text{C}, \tau_{S2}=0.67^\circ\text{C}, \tau_{S3}=1.30^\circ\text{C}$).
...
1. **Quasi-Steady Thermal Equilibrium**: Test durations ($15\text{--}60\text{ min}$) allow compact busbar/terminal assemblies to approach steady-state temperature rises relative to electrical transient time constants.
2. **Piecewise Linear Thermal Conduction**: Heat transfer from internal hotspot to external terminals adheres to Fourier's conduction law with effectively constant thermal conductivity over $20^\circ\text{C} \le T_{amb} \le 55^\circ\text{C}$.
...
2. **Virtual Sensor Imputation & State Estimation**: When a physical probe experiences contact degradation or fails, steady-state thermal regression estimators reconstruct virtual sensor values ($\hat{S}_i$), enabling uninterrupted test continuity.
3. **Physics-Informed Real-Time Thermal Tracking**: The trained ensemble executes sub-millisecond inference to track instantaneous hotspot rise $T_{hs}(t)$, feeding Arrhenius loss-of-life equations ($V = 2^{(T_{hs}-110)/6}$) to log dynamic insulation degradation.
